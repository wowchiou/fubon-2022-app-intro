...
function htmlForTest() {
  return src(srcFilePath.html) // 'app/*.html'
    .pipe($.fileInclude({ prefix: '##', basepath: '@file' }))
    .pipe($.useref({ searchPath: [Tmp, Src, '.'] })) // ['.tmp', 'app', '.']
    .pipe($.if(!isProd, dest(Tmp), dest(Dist))); // $.if(!isProd, dest('.tmp'), dest('dist'))
}
    ...
    src(srcFilePath.html) // 'app/*.html'
      ...
      .pipe(
        $.if(
          /\.js$/,
          $.uglify({
            mangle: { reserved: ['form', 'MG', 'auth2', 'stopBodyScroll'] },
...
const build = series(
  clean,
  parallel(
    series(lint, parallel(styles, scripts), htmlForBuild),
    images,
    fonts,
    lib,
    extras
...
function startAppServer() {
  ...
  watch(srcFilePath.html, htmlForTest); // 'app/*.html'
  watch(srcFilePath.component, htmlForTest); // 'app/components/**/*'
